...
typedef RT_MODEL_quarc_analog_loopbac_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * quarc_analog_loopback_demo(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(quarc_analog_loopback_demo_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://quarc_analog_loopback_demo:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: quarc_analog_loopback_demo -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
