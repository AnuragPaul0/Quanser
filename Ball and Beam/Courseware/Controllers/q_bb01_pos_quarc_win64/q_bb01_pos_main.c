...
typedef RT_MODEL_q_bb01_pos_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * q_bb01_pos(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_bb01_pos_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(q_bb01_pos_DW.HILReadAnalogTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://q_bb01_pos:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_bb01_pos -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(q_bb01_pos_DW.HILReadAnalogTimebase_Task, (t_clock)
        q_bb01_pos_P.HILReadAnalogTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
