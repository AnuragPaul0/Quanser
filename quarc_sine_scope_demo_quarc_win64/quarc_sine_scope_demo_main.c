...
typedef RT_MODEL_quarc_sine_scope_dem_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * quarc_sine_scope_demo(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(quarc_sine_scope_demo_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: quarc_sine_scope_demo -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
