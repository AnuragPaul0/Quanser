...
# ─────────────────────────────────────────────────────────────────────────────
#  Checkpointing  (features → .npz,  model → .joblib)
# ─────────────────────────────────────────────────────────────────────────────

def _feat_path(out_dir, split_name):
    safe = re.sub(r"[^\w]", "_", split_name)
    return os.path.join(out_dir, f"features_{safe}.npz")

def _meta_path(out_dir, split_name):
    safe = re.sub(r"[^\w]", "_", split_name)
    return os.path.join(out_dir, f"features_{safe}_meta.json")

def _model_path(out_dir):
    return os.path.join(out_dir, "model.joblib")


def save_features(out_dir, split_name, X, y, ok_real, ok_fake, stats):
    """Persist feature matrix + labels + path lists to disk."""
    np.savez_compressed(
        _feat_path(out_dir, split_name),
        X=X, y=y,
        ok_real=np.array(ok_real, dtype=object),
        ok_fake=np.array(ok_fake, dtype=object),
    )
    with open(_meta_path(out_dir, split_name), "w") as f:
        json.dump(stats, f, indent=2)
    print(f"  💾  Features saved  →  features_{re.sub(r'[^\\w]','_',split_name)}.npz")


def load_features(out_dir, split_name):
    """
    Load previously saved features.
    Returns (X, y, ok_real, ok_fake, stats) or None if not found.
    """
    fp = _feat_path(out_dir, split_name)
    mp = _meta_path(out_dir, split_name)
    if not (os.path.exists(fp) and os.path.exists(mp)):
        return None
    data     = np.load(fp, allow_pickle=True)
    X        = data["X"]
    y        = data["y"]
    ok_real  = data["ok_real"].tolist()
    ok_fake  = data["ok_fake"].tolist()
    with open(mp) as f:
        stats = json.load(f)
    print(f"  ✅  Loaded cached features  ←  {os.path.basename(fp)}"
          f"  ({(y==0).sum()} real | {(y==1).sum()} fake)")
    return X, y, ok_real, ok_fake, stats


def save_model(out_dir, clf):
    """Persist trained sklearn Pipeline with joblib."""
    path = _model_path(out_dir)
    joblib.dump(clf, path, compress=3)
    print(f"  💾  Model saved  →  model.joblib"
...
def load_model(out_dir):
    ...
    Returns clf or None if not found.
    """
    path = _model_path(out_dir)
    if not os.path.exists(path):
        return None
    clf = joblib.load(path)
    print(f"  ✅  Loaded cached model  ←  model.joblib")
    return clf
...
def build_split(cat_dirs, ethnicities, split_name,
                real_frames=32, fake_frames=1, out_dir=None):
    """
    Returns X, y, ok_real_paths, ok_fake_paths, per_eth_stats.

    If out_dir is given:
      - On first run: extracts features, saves  features_<split>.npz + _meta.json
      - On resume  : loads from those files and skips re-extraction entirely
    """
    # ── Try cache first ───────────────────────────────────────────────────────
    if out_dir:
        cached = load_features(out_dir, split_name)
        if cached is not None:
            return cached   # (X, y, ok_real, ok_fake, stats)
    ...
    # ── Save checkpoint ───────────────────────────────────────────────────────
    if out_dir:
        save_features(out_dir, split_name, X, y, ok_real, ok_fake, stats)
...
def main():
    ...
    ap.add_argument("--retrain", action="store_true",
                    help="Force re-train the model even if model.joblib exists")
    ap.add_argument("--reextract", action="store_true",
                    help="Force re-extract all features even if .npz caches exist "
                         "(implies --retrain)")
    args = ap.parse_args()

    if args.reextract:
        args.retrain = True   # stale features → stale model

    np.random.seed(args.seed)
    os.makedirs(args.out, exist_ok=True)

    # ── Wipe caches if reextract requested ───────────────────────────────────
    if args.reextract:
        for p in Path(args.out).glob("features_*.npz"):
            p.unlink(); print(f"  🗑  Removed {p.name}")
        for p in Path(args.out).glob("features_*_meta.json"):
            p.unlink()
        model_p = Path(_model_path(args.out))
        if model_p.exists():
            model_p.unlink(); print(f"  🗑  Removed model.joblib")

    elif args.retrain:
        model_p = Path(_model_path(args.out))
        if model_p.exists():
            model_p.unlink(); print(f"  🗑  Removed model.joblib  (--retrain)")
    ...
    # ── Model (cached after first train) ─────────────────────────────────────
    clf = load_model(args.out)
    if clf is None:
        print("\n🔬  Training RBF-SVM on training ethnicities ...")
        clf = Pipeline([
            ("scaler", StandardScaler()),
            ("svm",    SVC(kernel="rbf", C=10, probability=True, random_state=42))
        ])
        clf.fit(X_tr, y_tr)
        save_model(args.out, clf)
    else:
